...
for document in col.find({"$and":[ {"fr_abstract_s":{"$exists": True}}, {"fr_keyword_s":{"$exists": True}}]}):
    # Confirm if select conditions worked well
    if ('fr_abstract_s' in document) and ('fr_keyword_s' in document):
        if debug:
        ...
        for keyword in keywords:
            keyword_tokenized = nlp_fr(keyword)
        ...
        for word in keywords_lemmas:
            if (word in stopwords.words('french')) or (word in stopwords_homemade):
        ...
        ## Lemmatize summary
        abstract_tokenized = nlp_fr(abstract)
        abstract_lemmas = []
        ...
        for keyword in keywords_lemmas:
            if debug:
                ...
                print(keyword)
            keyword_syn = wordnet.synsets(keyword, lang="fra")
            ...
            for word in abstract_lemmas:
                ...
                # Else, compute the similarity using WOLF wordnet
                    ...
                    word_syn = wordnet.synsets(word, lang="fra")
